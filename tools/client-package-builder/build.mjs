...
function build(cfg) {
  ...
  put(
    ...
    [
      ...
      "chcp 65001 >nul",
      "setlocal",
      "title تثبيت وتجهيز Laundry ERP",
      "echo.",
      "echo   ============================================",
      `echo    Laundry ERP v${version} - تجهيز وتثبيت`,
      "echo   ============================================",
      "echo.",
      'set "RT=%APPDATA%\\@laundry\\runtime"',
      'if not exist "%RT%" mkdir "%RT%" >nul 2>&1',
      "echo   [1/2] نسخ إعداد التشغيل...",
      'copy /Y "%~dp02-التجهيز\\provision.json" "%RT%\\provision.json" >nul',
      "if errorlevel 1 (echo   فشل نسخ إعداد التشغيل. & pause & exit /b 1)",
      "echo         تم.",
      "echo.",
      "echo   [2/2] تشغيل المثبت...",
      `start /wait "" "%~dp01-التثبيت\\${setup}"`,
      "echo.",
      "echo   ============================================",
      "echo    اكتمل. شغّل البرنامج من أيقونة سطح المكتب.",
      "echo    ثم استورد ملف الترخيص من صفحة الترخيص.",
      "echo   ============================================",
      "echo.",
      "pause",
    ].join("\r\n"),
